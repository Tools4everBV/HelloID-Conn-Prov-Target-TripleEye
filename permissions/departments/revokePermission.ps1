#################################################################
# HelloID-Conn-Prov-Target-TripleEye-RevokePermission-Departments
# PowerShell V2
#################################################################
...
try {
    ...
    switch ($action) {
        'RevokePermission' {
            $body = @{
                ...
                departmentId = $actionContext.References.Permission.Reference + 1
            }
            $bodyJson = $body | ConvertTo-Json -Compress
            $headers['X-Signature'] = Get-Signature ($bodyJson)
            $splatUpdateParams = @{
                Uri         = "$($actionContext.Configuration.BaseUrl)/$($actionContext.Configuration.HookId)/organisation/unlinkDepartmentEmployee"
                Method      = 'PUT'
                Body        = $bodyJson
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoking TripleEye [Department] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                try {
                ...
                Write-Information "[DryRun] Revoke TripleEye [Department] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }

            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Revoke [Department] permission [$($actionContext.PermissionDisplayName)] was successful"
                    IsError = $false
                })
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not revoke TripleEye [Department] permission. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not revoke TripleEye [Department] permission. Error: $($_.Exception.Message)"
