################################################################
# HelloID-Conn-Prov-Target-TripleEye-GrantPermission-Departments
# PowerShell V2
################################################################
...
try {
    ...
    switch ($action) {
        'GrantPermission' {
            $body = @{
                ...
                departmentId = $actionContext.References.Permission.Reference
            }
            $bodyJson = $body | ConvertTo-Json -Compress
            $headers['X-Signature'] = Get-Signature ($bodyJson)
            $splatUpdateParams = @{
                Uri         = "$($actionContext.Configuration.BaseUrl)/$($actionContext.Configuration.HookId)/organisation/linkDepartmentEmployee"
                Method      = 'PUT'
                Body        = $bodyJson
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Granting TripleEye [Department] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                $null = Invoke-RestMethod @splatUpdateParams

            } else {
                Write-Information "[DryRun] Grant TripleEye [Department] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }

            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Grant [Department] permission [$($actionContext.PermissionDisplayName)] was successful"
                    IsError = $false
                })
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not grant TripleEye [Department] permission. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not grant TripleEye [Department] permission. Error: $($_.Exception.Message)"
