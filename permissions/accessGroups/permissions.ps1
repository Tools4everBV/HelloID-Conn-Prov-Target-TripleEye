############################################################
# HelloID-Conn-Prov-Target-TripleEye-Permissions-AccessGroups
# PowerShell V2
############################################################
...
try {
    Write-Information 'Retrieving permissions [AccessGroups]'
    $headers = @{
        token         = "$($actionContext.Configuration.Token)"
        'X-Signature' = Get-SignatureForEmptyBody
    }
    $getAccessGroups = @{
        Uri     = "$($actionContext.Configuration.BaseUrl)/$($actionContext.Configuration.HookId)/organisation/accessGroups"
        Method  = 'GET'
        Headers = $headers
    }
    $retrievedPermissions = Invoke-RestMethod @getAccessGroups
