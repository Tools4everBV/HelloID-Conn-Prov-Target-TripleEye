################################################################
# HelloID-Conn-Prov-Target-TripleEye-GrantPermission-AccessGroups
# PowerShell V2
################################################################
...
try {
    ...
    switch ($action) {
        'GrantPermission' {
            ...
            $splatUpdateParams = @{
                Uri         = "$($actionContext.Configuration.BaseUrl)/$($actionContext.Configuration.HookId)/organisation/linkAccessGroupEmployee"
                Method      = 'PUT'
                Body        = $bodyJson
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Granting TripleEye [AccessGroups] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                $null = Invoke-RestMethod @splatUpdateParams

            } else {
                Write-Information "[DryRun] Grant TripleEye [AccessGroups] permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }

            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Grant [AccessGroups] permission [$($actionContext.PermissionDisplayName)] was successful"
                    IsError = $false
                })
        }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not grant TripleEye [AccessGroups] permission. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not grant TripleEye [AccessGroups] permission. Error: $($_.Exception.Message)"
