#################################################
# HelloID-Conn-Prov-Target-TripleEye-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            Write-Information "Enabling TripleEye account with accountReference: [$($actionContext.References.Account)]"
            $bodyJson = @{
                employeeId     = $actionContext.References.Account
                accessDisabled = $false
                startStamp     = (Get-Date).ToUniversalTime().ToString('o')
                endStamp       = $null
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                Write-Information "[DryRun] Enable TripleEye account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable TripleEye account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not enable TripleEye account. Error: $($_.Exception.Message)"
