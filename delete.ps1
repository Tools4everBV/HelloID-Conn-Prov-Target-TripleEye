##################################################
# HelloID-Conn-Prov-Target-TripleEye-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            $splatDeleteParams = @{
                Uri         = "$($actionContext.Configuration.BaseUrl)/$($actionContext.Configuration.HookId)/organisation/employees/$($actionContext.References.Account)"
                Method      = 'DELETE'
                Body        = $bodyJson
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting TripleEye account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDeleteParams

            } else {
                Write-Information "[DryRun] Delete TripleEye account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete TripleEye account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete TripleEye account. Error: $($_.Exception.Message)"
